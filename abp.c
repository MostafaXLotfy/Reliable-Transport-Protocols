...
bool B_is_duplicate(const struct pkt *packet)
{
    if (receiver.lastReceived == NULL)
        return false;
    struct pkt *last_packet = receiver.lastReceived;
    if (
        strncmp(packet->payload, last_packet->payload, 20) != 0 ||
        last_packet->checksum != packet->checksum ||
        packet->acknum != last_packet->acknum ||
        packet->seqnum != last_packet->acknum)
    {
        return false;
    }
    free((void * )last_packet);
    return true;
}
...
void A_output(struct msg message)
{
    ...
    // making a packet from the passed message
    int checksum = calculate_checksum(message.data, sender.next_seqnum, sender.next_seqnum);
    sender.currentPacket = makePacket(sender.next_seqnum, message.data, checksum);
...
void A_input(struct pkt packet)
{
    ...
    if (isAcknowledged(&packet, sender.next_seqnum) && isNotCorrupted(&packet))
    {
        ...
        sender.next_seqnum = (sender.next_seqnum + 1) % 2;
...
void B_input(struct pkt packet)
{
    if (B_is_duplicate(&packet))
    {
        printf("[B] received duplicate ");
        print_message(packet.payload, 20);
        printf("\n");
        printf("[B] send Ack %d\n", packet.acknum);
        int checksum = calculate_checksum(NULL, packet.acknum, packet.acknum);
        struct pkt *tempPacket = makePacket(packet.acknum, NULL, checksum);
        tolayer3(1, *tempPacket);
        free((void *)tempPacket);
    }
    else if (isAcknowledged(&packet, receiver.expected_seqnum) && isNotCorrupted(&packet))
    {
        receiver.lastReceived = makePacket(packet.acknum, packet.payload, packet.checksum);
        // make Ack packet
